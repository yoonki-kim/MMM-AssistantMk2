...
Installer_checkaudio () {
			...
			while true; do
                if Installer_yesno "Checking audio output.\nMake sure your speakers are on press [Yes].\nPress [No] if you don't have speakers." true >/dev/null; then
					echo
					Installer_debug "Actual test input config: $play_hw"
                    aplay -D plug$play_hw "../resources/beep_check.wav" 2>/dev/null || Installer_error "Not working !"
                    Installer_yesno "Did you hear something?" true >/dev/null && break
                    echo
...
Installer_checkmic () {
            ...
            while true; do
                if Installer_yesno "Checking audio input.\nMake sure your microphone is on, press [Yes] and say something.\nPress [No] if you don't have a microphone." true >/dev/null; then
                    echo
                    Installer_debug "Actual test input config: $rec_hw"
                    arecord -D plug$rec_hw -r 16000 -c 1 -d 3 -t wav -f S16_LE $audiofile 2>/dev/null || Installer_error "Not Working !"
                    if [ -f $audiofile ]; then
